          ...
          <div className="ml-auto flex items-center gap-2">
            {user ? (
              <div className="hidden items-center gap-2 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] px-3 py-1.5 text-[11px] font-semibold md:flex">
                ...
                <span className="font-mono text-[10px] text-[hsl(var(--muted-foreground))]">{user.role}</span>
              </div>
            ) : null}
            {onLogout ? (
              <button type="button" onClick={onLogout} className="rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] px-3 py-2 text-xs font-bold hover:bg-[hsl(var(--muted))]">
                {lang === 'id' ? 'Keluar' : 'خروج'}
              </button>
            ) : null}
            ...
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-[hsl(var(--accent))] text-xs font-bold text-[hsl(var(--accent-foreground))]">
              {(user?.fullName || user?.username || 'RS').slice(0, 2).toUpperCase()}
            </div>
          </div>
        </header>
