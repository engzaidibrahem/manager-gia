...
function sourceChannel(source: string | null, admin: boolean): string {
  if (source === "qr") return "MOBILE_QR";
  if (source === "search") return "MOBILE_SEARCH";
  if (source === "admin" || admin) return "MOBILE_ADMIN";
...
export function OutPage() {
  ...
  const pickSearch = useQuery({
    queryKey: ["out-pick", debouncedPick],
  ...
  async function confirm() {
    ...
    try {
      const result = await postWarehouseOut({
        ...
        sourceChannel: sourceChannel(source, source === "admin"),
      });
      ...
      setSuccess("تم تأكيد الإخراج بنجاح.");
      setClientRequestId(null);
    } catch (e) {
      setFlash(e instanceof Error ? e.message : "فشل الإخراج");
    } finally {
      setPending(false);
    }
  }
  ...
  if (!Number.isFinite(itemId) || itemId <= 0) {
    return (
      <Screen title="إخراج" backTo="/">
        <p className="mb-3 text-sm text-[hsl(var(--muted-foreground))]">اختر منتجاً للإخراج</p>
        <input
        ...
        <div className="space-y-2">
          {(pickSearch.data?.rows ?? []).map((row) => (
            <button
              ...
              onClick={() => navigate(`/out?itemId=${row.id}&source=${source || "admin"}`)}
            >
  ...
  if (product.isLoading) {
    return (
      <Screen title="إخراج" backTo={`/product/${itemId}`}>
        <LoadingSpinner />
      </Screen>
    );
  }
  ...
  if (product.error || !detail) {
    return (
      <Screen title="إخراج" backTo="/search">
  ...
  return (
    <Screen title="إخراج من المستودع" backTo={`/product/${itemId}`}>
      ...
      {stockUnknown ? (
        <FlashBanner message="الرصيد غير معروف — لا يمكن الإخراج." />
      ) : reviewBlocked ? (
        <FlashBanner message="المنتج يحتاج مراجعة — لا يمكن الإخراج." />
      ) : overStock ? (
        <FlashBanner message="الكمية أكبر من الرصيد المتاح." />
      ) : null}
      ...
      {resultInfo ? (
        ...
        <div className="space-y-4">
          ...
          <button type="button" className="btn-lg btn-primary" disabled={!canConfirm} onClick={confirm}>
            {pending ? "جاري التأكيد..." : "تأكيد الإخراج"}
          </button>
        </div>
      )}
    </Screen>
